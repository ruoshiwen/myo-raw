...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            # Read notification handles corresponding to the for EMG characteristics
                ...
                self.emg_list = self.on_emg(emg1, 0, self.emg_list)
                self.emg_list = self.on_emg(emg2, 0, self.emg_list)
    ...
    def start_raw(self):
        ...
        self.write_attr(0x19, b'\x01\x03\x03\x01\x01')
    ...
    def on_emg(self, emg, moving, emg_list):
        for h in self.emg_handlers:
            return h(emg, moving, emg_list)
...
if __name__ == '__main__':
    ...
    filename = '/home/ruoshi/myo-raw/data/extension0.npy'

    def proc_emg(emg, moving, times=[], emg_list=[]):
        
        if HAVE_PYGAME:
            # update pygame display
            plot(scr, [e / 500. for e in emg])
        else:
            emg_list.append(list(emg))
            print('emg',emg)
            return emg_list
    ...
    # press ctrl+C to terminate the data acquisition process, and the raw EMG will be saved to a npy file
    except KeyboardInterrupt:
        emg_array = np.array(m.emg_list, dtype = int)
        np.save(filename, emg_array)
    finally:
        # m.power_off()
        # print("Power off")
        m.disconnect()
