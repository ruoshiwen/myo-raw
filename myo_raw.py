...
if __name__ == '__main__':
    ...
    def proc_emg(emg, moving, times=[]):
        if HAVE_PYGAME:
            ...
            filename = 'extension0.npy'
            y = np.load(filename) if os.path.isfile(filename) else [] #get data if exist
            np.save(filename,np.append(y,np.array(emg, dtype = int))) #save the new emg 
            print(emg)
